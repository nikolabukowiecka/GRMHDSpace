...
int main()
{
    ...
    for (int i = 0; i <= (cell_number * 0.3); i++)
    {
        density[i] = 1.0;
        vel_x[i] = 0.75;
        vel_y[i] = 0.0;
    ...
    for (int i = ((cell_number * 0.3) + 1); i < cell_number; i++)
    {
        density[i] = 0.125;
        vel_x[i] = 0.0;
        vel_y[i] = 0.0;
    ...
    while (t < final_time)
    {
        ...
        for (int i = 2; i < (cell_number + 2); i++)
        {
            ...
            vector<double> consVar_left_left = cons(density_padded[i - 2], vel_x_padded[i - 2], vel_y_padded[i - 2], vel_z_padded[i - 2], energy_tot[i - 2]);
            vector<double> consVar_right = cons(density_padded[i + 1], vel_x_padded[i + 1], vel_y_padded[i + 1], vel_z_padded[i + 1], energy_tot[i + 1]);
            vector<double> consVar_right_right = cons(density_padded[i + 2], vel_x_padded[i + 2], vel_y_padded[i + 2], vel_z_padded[i + 2], energy_tot[i + 2]);
